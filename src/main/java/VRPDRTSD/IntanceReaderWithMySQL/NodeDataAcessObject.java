...
public class NodeDataAcessObject {
    ...
    public void addNodeIntoDataBase(Node node) {
        String sql = "insert into nodes_test "
    ...
    public List<Node> getListOfNodes() {
        try {
            ...
            PreparedStatement stmt = this.connection.prepareStatement("select * from nodes_test");

            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
               Node node = new Node(rs.getInt("node_id"), rs.getDouble("latitude"), rs.getDouble("longiture"));               
